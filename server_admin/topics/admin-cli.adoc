...
[options="nowrap"]
----
$ kcadm.sh config credentials --server http://localhost/auth/admin/aerobase/console --realm demo --user admin --client admin
...
[options="nowrap"]
----
c:\> kcadm config credentials --server http://localhost/auth/admin/aerobase/console --realm demo --user admin --client admin
...
[options="nowrap"]
----
$ kcadm.sh config credentials --server http://localhost/auth/admin/aerobase/console --realm master --user admin --password admin
----
...
[options="nowrap"]
----
$ kcadm.sh get realms --no-config --server http://localhost/auth/admin/aerobase/console --realm master --user admin --password admin
----
...
For example, if you authenticate against the server http://localhost/auth/admin/aerobase/console and realm is [filename]`master`, then using [filename]`users` as ENDPOINT results in the resource URL http://localhost/auth/admin/aerobase/console/admin/realms/master/users.

If you set ENDPOINT to [filename]`clients`, the effective resource URI would be http://localhost/auth/admin/aerobase/console/admin/realms/master/clients.
...
In this example, you start a session authenticated as the [filename]`admin` user in the [filename]`master` realm. You then perform a POST call against the resource URL [filename]`http://localhost/auth/admin/aerobase/console/admin/realms/demorealm/users`.
...
[options="nowrap"]
----
$ kcadm.sh get http://localhost/auth/admin/aerobase/console/admin/realms/demorealm/clients/8f271c35-44e3-446f-8953-b0893810ebe7/installation/providers/docker-v2-compose-yaml -r demorealm > keycloak-docker-compose-yaml.zip
----
